 ...
 *
 */
class Q18(lineitem: DataFrame, customer: DataFrame, order: DataFrame, part: DataFrame, partsupp: DataFrame, nation: DataFrame, region: DataFrame, supplier: DataFrame) extends TpchQuery(lineitem, customer, order, part, partsupp, nation, region, supplier) {
  ...
  override def execute(sc: SparkContext): DataFrame = {
    ...
    l_lineitem.groupBy($"l_orderkey")
      .agg(sum($"l_quantity").as("sum_quantity"))
      .filter($"sum_quantity" > 300)
      .select($"l_orderkey".as("key"), $"sum_quantity")
      .join(o_order, o_order("o_orderkey") === $"key")
      .join(l_lineitem, $"o_orderkey" === l_lineitem("l_orderkey"))
      .join(c_customer, c_customer("c_custkey") === $"o_custkey")
