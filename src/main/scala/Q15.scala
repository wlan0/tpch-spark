 ...
 *
 */
class Q15(lineitem: DataFrame, customer: DataFrame, order: DataFrame, part: DataFrame, partsupp: DataFrame, nation: DataFrame, region: DataFrame, supplier: DataFrame) extends TpchQuery(lineitem, customer, order, part, partsupp, nation, region, supplier) {
  ...
  override def execute(sc: SparkContext): DataFrame = {
    ...
    val revenue = l_lineitem.filter($"l_shipdate" >= "1996-01-01" &&
    ...
    revenue.agg(max($"total").as("max_total"))
      ...
      .join(s_supplier, $"l_suppkey" === s_supplier("s_suppkey"))
