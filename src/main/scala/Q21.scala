 ...
 *
 */
class Q21(lineitem: DataFrame, customer: DataFrame, order: DataFrame, part: DataFrame, partsupp: DataFrame, nation: DataFrame, region: DataFrame, supplier: DataFrame) extends TpchQuery(lineitem, customer, order, part, partsupp, nation, region, supplier) {
  ...
  override def execute(sc: SparkContext): DataFrame = {
    ...
    val fsupplier = s_supplier.select($"s_suppkey", $"s_nationkey", $"s_name")

    val plineitem = l_lineitem.select($"l_suppkey", $"l_orderkey", $"l_receiptdate", $"l_commitdate")
    //cache
    ...
    val forder = o_order.select($"o_orderkey", $"o_orderstatus")
      .filter($"o_orderstatus" === "F")

    n_nation.filter($"n_name" === "SAUDI ARABIA")
