 ...
 *
 */
class Q13(lineitem: DataFrame, customer: DataFrame, order: DataFrame, part: DataFrame, partsupp: DataFrame, nation: DataFrame, region: DataFrame, supplier: DataFrame) extends TpchQuery(lineitem, customer, order, part, partsupp, nation, region, supplier) {
  ...
  override def execute(sc: SparkContext): DataFrame = {
    ...
    c_customer.join(o_order, $"c_custkey" === o_order("o_custkey")
      && !special(o_order("o_comment")), "left_outer")
      .groupBy($"o_custkey")
