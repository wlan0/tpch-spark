 ...
 *
 */
class Q03(lineitem: DataFrame, customer: DataFrame, order: DataFrame, part: DataFrame, partsupp: DataFrame, nation: DataFrame, region: DataFrame, supplier: DataFrame) extends TpchQuery(lineitem, customer, order, part, partsupp, nation, region, supplier) {
  ...
  override def execute(sc: SparkContext): DataFrame = {
    ...
    val fcust = c_customer.filter($"c_mktsegment" === "BUILDING")
    val forder = o_order.filter($"o_orderdate" < "1995-03-15")
    val flineitems = l_lineitem.filter($"l_shipdate" > "1995-03-15")

    fcust.join(forder, $"c_custkey" === forder("o_custkey"))
